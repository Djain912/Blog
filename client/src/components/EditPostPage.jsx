...
const EditPostPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        const response = await fetch(`http://localhost:9000/getposts/${id}`);
        const data = await response.json();
        setPost(data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:9000/updatepost/${id}`, {
        method: 'PUT',
        headers: {
          ...
          <div className="mb-4 flex items-center">
            ...
            {previewUrl && (
              <div className="relative">
                <img src={previewUrl} alt="Preview" className="w-32 h-32 object-cover rounded-md mb-2" />
                {isUploadingImage && (
